...
const OUTPUT_DIR = path.join(ROOT, 'output', 'food');
...
function compareStructure(newArticle, oldArticle) {
  ...
  const KEY_FIELDS = [
    ...
    'sourceId', 'dish_type', 'signature_ingredients',
    'formats',
  ];
  for (const f of KEY_FIELDS) {
    const inNew = f in newArticle;
    const inOld = f in oldArticle;
    if (inNew !== inOld)
      diffs.push(`campo "${f}": presente nel nuovo=${inNew}, nel vecchio=${inOld}`);
  }
  // formats sub-keys
  for (const ch of ['x', 'instagram', 'tiktok']) {
    const inNew = !!(newArticle.formats?.[ch]);
    const inOld = !!(oldArticle.formats?.[ch]);
    if (inNew !== inOld)
      diffs.push(`formats.${ch}: presente nel nuovo=${inNew}, nel vecchio=${inOld}`);
  }
  return diffs;
}

async function main() {
  console.log('=== verify-food-migration.js ===\n');

  // 1. Snapshot prima
  fs.mkdirSync(OUTPUT_DIR, { recursive: true });
  const beforeSet = new Set(fs.readdirSync(OUTPUT_DIR).filter(f => f.endsWith('.json')));

  // 2. Esegui il runner (timeout 10min — il caricamento dei moduli trigger RSS fetch IIFEs)
  console.log('→ MAX_NEW_FOOD_ARTICLES=1 node core/run-agent.js food\n');
  const run = spawnSync('node', ['core/run-agent.js', 'food'], {
    env:      { ...process.env, MAX_NEW_FOOD_ARTICLES: '1' },
    cwd:      ROOT,
    timeout:  600000,
    encoding: 'utf8',
  });
  ...
  if (newFiles.length === 0) {
    console.log('Nessun nuovo articolo generato (feed senza novità o gate fallito).');
    ...
    // Confronto strutturale con articolo esistente
    const existingFiles = afterFiles.filter(f => beforeSet.has(f)).sort().reverse();
    if (existingFiles.length > 0) {
      const oldArticle = JSON.parse(fs.readFileSync(path.join(OUTPUT_DIR, existingFiles[0]), 'utf8'));
      const diffs = compareStructure(newArticle, oldArticle);
      if (diffs.length > 0) {
